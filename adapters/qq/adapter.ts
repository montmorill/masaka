...
function parseGroupMember({ member_openid }: QQ.GroupMemberEvent): Satori.GuildMember {
  return { user: { id: member_openid } }
}

function parseGroupMemberAdd(event: QQ.GroupMemberEvent): Satori.Events['guild-member-added'] {
  return { guild: { id: event.group_openid }, member: parseGroupMember(event) }
}

function parseGroupMemberRemove(event: QQ.GroupMemberEvent): Satori.Events['guild-member-removed'] {
  return { guild: { id: event.group_openid }, member: parseGroupMember(event) }
}

function parseGroupJoinRequest(event: QQ.GroupJoinRequest): Satori.Events['guild-member-request'] {
  return {
...
function parseFriendAdd({ openid }: QQ.FriendAdd): Satori.Events['friend-request'] {
  return { user: { id: openid } }
}

function parseUser(user: QQ.User): Satori.User {
  return {
    id: user.id,
    name: user.username,
    isBot: user.bot,
...
function parseMember(member: QQ.Member): Satori.GuildMember {
  return {
    user: parseUser(member),
  }
}

function parseMessageScene(scene: QQ.MessageScene): Record<string, string> {
...
function parseArkData({ prompt, ark_type, ark_name, fields }: QQ.ArkData): Element<'qq:ark'> {
  return h['qq:ark']({ ark_type, ark_name, prompt, ...snakeCaseKeys(fields) })
}

function parseMentions(content: string, mentions: NonNullable<QQ.GroupMessage['mentions']>): Fragment {
...
function parseImageAttachment(attrs: Element<'image'>['attrs'], {
  width,
  height,
  content,
  ...attachment
...
function parseAttachments(attachments: QQ.Attachment[]): Element<'file' | 'audio' | 'image' | 'video'>[] {
  const elements = []
  for (const { url, filename, size, ...attachment } of attachments) {
    ...
    if (attachment.content_type === 'file')
      ...
      elements.push(parseImageAttachment(attrs, attachment))
    else
...
function parseMsgElements(msg_elements: QQ.MsgElement[], ref_msg_idx: QQ.RefMsgIdx): Element<'quote'> {
  ...
  for (const { ...msg_element } of msg_elements) {
    ...
    if (msg_element.attachments)
      ...
      element.children.push(parseArkData(msg_element.ark_data).update(msg_element.content))
...
function parseMessageContent({
  ...
  const { ref_msg_idx, ...scene } = parseMessageScene(message_scene)
  ...
  if (message_type === QQ.MessageType.Ark)
    content = parseArkData(message.ark_data).update(message.content)
    ...
    logger.warn('unknown message type', message_type, message)
  if (message.mentions)
    content = parseMentions(message.content, message.mentions)
  if (message_type === QQ.MessageType.Quote)
    element.children.push(parseMsgElements(message.msg_elements, ref_msg_idx as QQ.RefMsgIdx))
  const attachments = message.attachments ? parseAttachments(message.attachments) : []
...
function parseUserMessage(message: QQ.Message): Satori.Events['message-created'] {
  const sender = parseUser(message.author)
  if (sender.name === '')
    delete sender.name
  else
    logger.warn('unexpected C2C_MESSAGE_CREATE author.username', sender.name)
  const element = parseMessageContent(message)
...
function parseGroupMessage(message: QQ.GroupMessage): Satori.Events['message-created'] {
  const member = parseMember(message.author)
  const element = parseMessageContent(message)
  return {
    message: messageResource(element),
...
function messageResource(element: Element<'message'>): Satori.Message {
