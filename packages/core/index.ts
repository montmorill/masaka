...
export * from './message'
export * from './session'
export type { Fragment } from '@yak/element'
export * as Universal from '@yak/protocol'
export * from 'cordis'
...
declare module '@cordisjs/plugin-http' {
  namespace HTTP {
    export function createConfig(this: typeof HTTP, endpoint?: string | boolean): Schema<Config>
  }
}

HTTP.createConfig = function createConfig(this, endpoint) {
  return Schema.object({
    endpoint: Schema.string().role('link').description('要连接的服务器地址。').default(typeof endpoint === 'string' ? endpoint : undefined!).required(typeof endpoint === 'boolean' ? endpoint : false),
...
export class Satori<C extends Context = Context> extends Service<unknown, C> {
  ...
  constructor(ctx: C) {
    super(ctx)
    ctx.mixin('satori', ['bots'])

    defineProperty(this.bots, Service.tracker, {})

    // const handleInternalRoute = this.handleInternalRoute.bind(this)

    // ctx.on('http/file', async (_url, options) => {
    //   const url = new URL(_url)
    //   if (url.protocol !== 'internal:')
    //     return
    //   const { status, body, headers } = await handleInternalRoute('GET', url)
    //   if (status >= 400)
    //     throw new Error(`Failed to fetch ${_url}, status code: ${status}`)
    //   if (status >= 300) {
  ...
  //     return session.transform(result)
  //   }
